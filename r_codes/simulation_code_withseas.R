...
# hseFactor - proportion of mosquitoes diverted elsewhere other than to a house
  ...
  for (t in 1:numDays) {
	...
	# When data on mosquitoes being diverted elsewhere exists     
    # derive the proportion of mosquitoes that are diverted elsewhere for each house
     for (h in 1:30) {
       mosqData1$dissap[h]<- (mosqData1$outgoing[h]) * (1-hseFactor)
     }
    
	 mosqData1$dissap[is.na(mosqData1$dissap)]<-0.00001 #to avoid missing values in denominator

	# obtain the adjusted predicted mosquitoes when data on mosquitoes being diverted elsewhere exists 
     mosqData1$adjpredp <- (mosqData1$predp/sum(mosqData1$predp+mosqData1$dissap))

    # obtain the total predicted mosquitoes diverted elsewhere from all households
	 mosqData1$adjdis <- (mosqData1$dissap/sum(mosqData1$predp+mosqData1$dissap))
	
	# simulate observed mosquitoes in each house from a multinomial distribution
	 observedMosq<-rmultinom(1,NmosqPerDay,c(mosqData1$adjpredp,sum(mosqData1$adjdis)))
     mosqData1$obsmosq<-observedMosq[1:30]
