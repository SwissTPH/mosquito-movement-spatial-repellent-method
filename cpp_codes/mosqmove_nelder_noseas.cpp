// mosqMoveGithub.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...

{
	...
	int combo[2160]; // whether the household has a treated coil (combo=1) or not (combo=0)
	int notmissing[2160];
	...
	for (int d = 0; d < numDays; d++) {
		...
		// sets up houses h1 for the pairs (h1, h2)
	...
	// baseline distribution of mosquitoes with zero coverage of spatial repellent, given as the proportion per house 
	for (int h = 0; h < 30; h++) {
		outfile << h << " " << pr[h] << " "  << endl;
	}

	outfile << "repFactor " << spatialRepFactor << " " << endl; // proportion of mosquitoes repelled from households using repellents
	outfile << "lambda " << lambda << " " << endl; // parameter for distribution of distances moved between houses (for the half-normal chosen the mean = lambda*sqrt(2/pi) )
