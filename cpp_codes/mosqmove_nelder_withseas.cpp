...
for (int h=0; h<30; h++) {
    //the number of mosquitoes repelled but not going to a household
    dissaph[h] = (outgoingh[h]) * (1- hseFactor);
...
for (int h=0; h<30; h++) {
  ...
  dissapear = ntotal - sumobsmosq;

//sum the total number of mosquitoes diverted elsewhere from each house
double sumdiss = 0.0;
for (int h=0; h<30; h++) {
    sumdiss = sumdiss + dissaph[h];
}

 // sets up houses with P30 for the observed mosquitoes
double obsmosqa[31];
double adjpredpha[31];

 // simulated numbers of mosquitoes and the adjusted predicted probability accounting for mosquitoes diverted elsewhere
for (int h=0; h<30; h++) {
	obsmosqa[h] = obsmosq[h + (d*30)]; 
	adjpredpha[h] = adjpredph[h]; //add the disappearing mosquitoes
 }

 for (int h=30; h<31; h++) {
     obsmosqa[h] = dissapear;
     adjpredpha[h] = sumdiss/sumpredph;
 }
  ...
  for (int h=0; h<31; h++) {
    ...
    if (adjpredpha[h]>0) {
           loglik = loglik + (obsmosqa[h] *log(adjpredpha[h]));
...
for (int h=0; h<30; h++) {
 ...
 // simulated numbers of mosquitoes and the adjusted predicted probability accounting for mosquitoes diverted elsewhere
for (int h=0; h<31; h++) {
	outfile << h << " " << obsmosqa[h] << "  "<< adjpredpha[h] << "  "<< endl;
  }
